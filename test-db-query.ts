...
async function testDatabase() {
  ...
  try {
    ...
    const articlesByType = allArticles.reduce(
      (acc, article) => {
    ...
    const articlesByStatus = allArticles.reduce(
      (acc, article) => {
    ...
    if (guideArticle) {
      ...
      console.log(
        `   - Contenu (premiers 100 caractères): ${guideArticle.content.substring(0, 100)}...`,
      );
      console.log(
        `   - Contient des tableaux Markdown: ${guideArticle.content.includes("|") ? "Oui" : "Non"}`,
      );
      console.log(
        `   - Contient des H3: ${guideArticle.content.includes("###") ? "Oui" : "Non"}`,
      );
    } else {
