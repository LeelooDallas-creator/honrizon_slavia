...
export function initInteractiveCheckboxes() {
  ...
  const savedState = JSON.parse(localStorage.getItem(`checklist-${pageKey}`) || '{}');
  ...
  taskListItems.forEach((li, index) => {
    ...
    checkbox.addEventListener('change', () => {
      const currentState = JSON.parse(localStorage.getItem(`checklist-${pageKey}`) || '{}');
      currentState[index] = (checkbox as HTMLInputElement).checked;
      localStorage.setItem(`checklist-${pageKey}`, JSON.stringify(currentState));
    });
  });
}
